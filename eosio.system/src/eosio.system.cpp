...
namespace eosiosystem {
   ...
   void system_contract::sethomepage( name account, std::string url){
      eosio_assert( url.size() <= 256, "url is too long");
      eosio_assert( url.find("http") == 0, "illegal url");
      setpersonal( account, "homepage", url);
   }

   void system_contract::setpersonal(name account,std::string key,std::string value){
...
EOSIO_DISPATCH( eosiosystem::system_contract,
     ...
     (rmvproducer)(updtrevision)(bidname)(bidrefund)(sethomepage)(setpersonal)
