...
#include <eosiolib/transaction.h>
    ...
    void randomtest::apply( name code, name act ) {
        if( code != _self )
            return;
    ...
    void randomtest::hascontract(const args_name& t){
...
//        }
    ...
    void randomtest::generate(const args& t){
    ...
    void randomtest::inlineact(const argsinline& t){
        auto& payer = t.payer;
        args gen;
        gen.loop = 1;
        gen.num = 1;
        ...
        if(t.in != ""_n)
        {
            INLINE_ACTION_SENDER(spaceaction::randomtest, generate)( t.in, {payer,"active"_n},
                                                               { gen});
            INLINE_ACTION_SENDER(spaceaction::randomtest, generate)( t.in, {payer,"active"_n},
                                                                     { gen});
        }
...
//     spaceaction::randomtest obj(receiver);
...
EOSIO_DISPATCH_CUSTOM(randomtest, (apply)(generate)(clear)(hascontract)(inlineact))
